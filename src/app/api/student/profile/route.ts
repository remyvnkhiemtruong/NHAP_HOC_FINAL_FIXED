...
function effectiveValue(value: ProfileValue | undefined): string {
  if (!value) return "";
  if (["ACCEPTED", "ADMIN_EDITED"].includes(value.change_status) && value.approved_value !== null) {
...
function toAdmissionPayload(
  ...
  return {
    ...
    middleSchool: effectiveValue(values.get(ADMISSION_FIELD_CODES.middleSchool)),
    middleSchoolCommune: effectiveValue(values.get(ADMISSION_FIELD_CODES.middleSchoolCommune)),
    fourYearAverage: effectiveValue(values.get(ADMISSION_FIELD_CODES.fourYearAverage)),
    fourYearConduct: effectiveValue(values.get(ADMISSION_FIELD_CODES.fourYearConduct)),
    priorityScore: effectiveValue(values.get(ADMISSION_FIELD_CODES.priorityScore)),
    encouragementScore: effectiveValue(values.get(ADMISSION_FIELD_CODES.encouragementScore)),
    admissionScore: effectiveValue(values.get(ADMISSION_FIELD_CODES.admissionScore)),
    note: effectiveValue(values.get(ADMISSION_FIELD_CODES.note)),
  };
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const profileValues: ProfileValue[] = student.profile_values.length
      ...
      : profileSeedRows(student.id, student.admission_record).map((row) => ({
          ...row,
          approved_value: null,
        }));
    ...
    for (const value of profileValues) {
      if (!value.field_code.startsWith("ADMISSION_")) fields[value.field_code] = effectiveValue(value);
    }
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const getValue = (code: string) =>
        code in fieldsToUpdate ? fieldsToUpdate[code] : effectiveValue(byCode.get(code));
      const validationError = validateStudentChanges(fieldsToUpdate, getValue("F"));
      ...
      const writeProposal = async (code: string, value: string) => {
        ...
        if (effectiveValue(existing) === value) return;
      ...
      const admissionValues = Object.fromEntries(
        Object.entries(ADMISSION_FIELD_CODES).map(([name, code]) => [name, effectiveValue(byCode.get(code))]),
